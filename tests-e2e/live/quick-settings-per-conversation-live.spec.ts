...
import { bootstrapLiveAPI, operateQuickSettings, TEST_MODEL_REGEX } from './helpers';
...
test('Live: per-conversation Quick Settings persist across switches', async ({ page }) => {
  ...
  await operateQuickSettings(page, { mode: 'ensure-open', model: TEST_MODEL_REGEX, reasoningEffort: 'low', verbosity: 'low', closeAfter: true });
  ...
  // conv3: use TEST_MODEL (the centralized test model)
  await operateQuickSettings(page, { mode: 'ensure-open', model: TEST_MODEL_REGEX, reasoningEffort: 'low', verbosity: 'low' });
  ...
  await operateQuickSettings(page, { mode: 'ensure-open', model: TEST_MODEL_REGEX, reasoningEffort: 'high', verbosity: 'high', summary: 'auto', closeAfter: true });

  // conv1 settings
  await rows.nth(2).click({ force: true });
  await operateQuickSettings(page, { mode: 'ensure-open', model: TEST_MODEL_REGEX, reasoningEffort: 'medium', verbosity: 'medium', summary: 'null', closeAfter: true });
  ...
  // We use TEST_MODEL as the standard test model, and we can assert reasoning control values
