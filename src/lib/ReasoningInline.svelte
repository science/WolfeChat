...
{#if windowsForAnchor.length > 0}
  {#each windowsForAnchor as w (w.id)}
    <details class="my-3 rounded border border-gray-500 bg-primary shadow-sm open:shadow-md" open={w.open} role="region" aria-label="Reasoning window for {w.model || 'assistant'}" data-reasoning-window-id={w.id}>
