// Auto-generated test entry point
export { runAllTests, formatSuiteResultsText, clearTests } from './src/tests/testHarness.js';
import './src/tests/unit/anthropic-client-factory.test.ts';
import './src/tests/unit/anthropic-conversion.test.ts';
import './src/tests/unit/anthropic-model-config.test.ts';
import './src/tests/unit/anthropic-reasoning-integration.test.ts';
import './src/tests/unit/anthropic-reasoning-modelconfig-integration.test.ts';
import './src/tests/unit/anthropic-reasoning-support.test.ts';
import './src/tests/unit/anthropic-sdk-conversion.test.ts';
import './src/tests/unit/anthropic-sdk-import.test.ts';
import './src/tests/unit/anthropic-sdk-messaging-maxtoken-fix.test.ts';
import './src/tests/unit/anthropic-sdk-streaming.test.ts';
import './src/tests/unit/anthropic-service-mock.test.ts';
import './src/tests/unit/claude-integration.test.ts';
import './src/tests/unit/claude-reasoning-settings.test.ts';
import './src/tests/unit/claude-thinking-toggle.test.ts';
import './src/tests/unit/codeRendererStreaming.unit.test.ts';
import './src/tests/unit/complete-model-flow-simulation.test.ts';
import './src/tests/unit/conversation-quick-settings-thinking.test.ts';
import './src/tests/unit/conversation-switching-race.test.ts';
import './src/tests/unit/ctrlEnterSend.test.ts';
import './src/tests/unit/debug-logging-control.test.ts';
import './src/tests/unit/debug-store-state.test.ts';
import './src/tests/unit/debugLayerLogger.unit.test.ts';
import './src/tests/unit/dom-dropdown-state-analysis.test.ts';
import './src/tests/unit/draft-isolation.test.ts';
import './src/tests/unit/e2e-helper-validation.test.ts';
import './src/tests/unit/gpt-5.1-reasoning-none.test.ts';
import './src/tests/unit/icon-styling.test.ts';
import './src/tests/unit/intentional-failure-demo.test.ts';
import './src/tests/unit/keyboardSettings.test.ts';
import './src/tests/unit/messageAssemblyDebugLogging.unit.test.ts';
import './src/tests/unit/messageModelLabeling.unit.test.ts';
import './src/tests/unit/multiple-summaries.test.ts';
import './src/tests/unit/provider-model-filtering.test.ts';
import './src/tests/unit/providerStore-json-parse.test.ts';
import './src/tests/unit/reasoning-cleanup.test.ts';
import './src/tests/unit/reasoning-window-conversation-id.test.ts';
import './src/tests/unit/reasoningAutoCollapseStore.test.ts';
import './src/tests/unit/reasoningPayload.test.ts';
import './src/tests/unit/reasoningStoreDebugLogging.unit.test.ts';
import './src/tests/unit/responsesConversionAndPayload.test.ts';
import './src/tests/unit/sdk-messaging-thinking-config.test.ts';
import './src/tests/unit/settings-provider-filtering-tdd.test.ts';
import './src/tests/unit/sseParserDebugLogging.unit.test.ts';
import './src/tests/unit/storeUpdatesDebugLogging.unit.test.ts';
import './src/tests/unit/streaming-error-recovery.test.ts';
import './src/tests/unit/summary-filtering.test.ts';
import './src/tests/unit/summary-generation.test.ts';
import './src/tests/unit/summary-loading-style.test.ts';
import './src/tests/unit/summary-manager-model.test.ts';
import './src/tests/unit/summary-message-model-display.test.ts';
import './src/tests/unit/summary-model-selection.test.ts';
import './src/tests/unit/summary-model-store.test.ts';
import './src/tests/unit/summary-streaming-alignment.test.ts';
import './src/tests/unit/summary-streaming.test.ts';
import './src/tests/unit/summary-word-target.test.ts';
import './src/tests/unit/test-harness-quiet-output.test.ts';
import './src/tests/unit/textareaSettings.test.ts';
import './src/tests/unit/title-model-utils.test.ts';